...
Tuples are like lists but are immutable like strings. They are indexed starting at 0. Instead of using [] when assigning values use () for tuples - though you still use [] when referencing an index. Of the methods and functions used with lists count and index can be used with tuples. Because tuples are immutable they can be used as keys for dictionaries. \
...
+ We then use the SELECT command to read the title and plays columns from the table Tracks. We can then loop through the rows in the cursor.
+ In actuality the cursor loads the rows as we ask for them (otherwise it would be impossible to search through the contents of a database due to memory constraints)
+ The rows we get from the cursor are in the form of Python tuples
+ Python2 code will have strings that start with u' indicating unicode strings rather than the byte based strings python2 used for strings.
+ The last command DELETES the two rows we wrote to the table, this shows the ability of SQL to express selection criteria.

**** 15.5 SQL
*** Video part 1
SQL abstracts the details imparitive lamguage
